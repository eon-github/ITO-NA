...
function sendOwnerReply(event) {
    ...
    const ownerComment = formData.get('owner-comment');
    const commentId = formData.get('ownerid');
    const restoName = formData.get('ownerRestoName');
    ...
    .then(data => {
        console.log('Owner reply sent successfully:', data);
        // Optionally, you can perform any actions after successful reply, such as updating UI
    })
    .catch(error => {
        console.error('Error sending owner reply:', error);
        // Optionally, you can handle errors here, such as displaying an error message to the user
    });
}

// Get all forms with the name 'owner-reply'
const ownerReplyForms = document.querySelectorAll('form[name="owner-reply"]');

// Attach the sendOwnerReply function to each form
ownerReplyForms.forEach(form => {
    form.addEventListener('submit', sendOwnerReply);
});
