    ...
    try {
      ...
      bcrypt.compare(passlogin, user.password, (err, isValid) => {
        ...
        if (isValid) { // Password is correct
          ...
          if (remember == null){
            res.cookie('user', userlogin);
          } else {
            options = {
              maxAge: 30 * 24 * 60 * 60 * 1000, // 30 days or 0 (session cookie)
...
app.post('/owner-reply', (req, res) => {
    const { owncomment, commentId } = req.body; // Extract data from request body
    console.log(req.body)


    // Find the comment by its ID and update it with the owner's reply
    commentModel.findById(commentId)
        .then(comment => {
            if (!comment) {
                console.log('Comment not found');
                res.status(404).json({ error: 'Comment not found' }); // Handle case where comment is not found
            } else {
                // Update the comment with the owner's reply
                comment.ownerReply = owncomment;
                comment.ownerReplyStatus = 1;
        ...
        .then(updatedComment => {
            console.log('Owner reply added successfully:', updatedComment);
            res.status(200).json({ message: 'Owner reply added successfully', comment: updatedComment });
            // Optionally, you can perform any actions after adding the owner's reply
        })
        .catch(error => {
            console.error('Error adding owner reply:', error);
            res.status(500).json({ error: 'Error adding owner reply' });
